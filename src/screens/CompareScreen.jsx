...
import { View, Text, Button, StyleSheet, ScrollView, Alert, TouchableOpacity, Platform, Dimensions } from 'react-native';
...
const CompareScreen = () => {
    ...
    const processComparison = (data) => {
        const processDay = (dayData) => {
            ...
            return {
                ...
                min: Math.min(...temps),
                count: temps.length,
                temps: temps // Guardamos temps para la gráfica (simplificado, idealmente con hora)
            };
        };
    ...
    const TimePickerBtn = ({ label, time, onPress }) => (
        <TouchableOpacity style={styles.pickerBtn} onPress={onPress}>
            <Text style={styles.pickerLabel}>{label}</Text>
            <View style={styles.pickerValueContainer}>
    ...
    // Función auxiliar para generar datos de gráfica si hay suficientes puntos
    const getChartData = (data1, data2, label1, label2) => {
        // Simplificación: Tomamos hasta 10 puntos distribuidos uniformemente para la gráfica
        // Esto es porque los arrays pueden tener diferente longitud
        const normalize = (arr) => {
        ...
        return {
            labels: Array.from({ length: Math.max(d1.length, d2.length) }, (_, i) => i + 1), // Indices simples
            datasets: [
    ...
    return (
        <ScrollView style={styles.container}>
            <Text style={styles.title}>Comparar Fechas</Text>

            <View style={styles.filterCard}>
                <View style={styles.rowContainer}>
                    <DatePickerBtn label="Fecha 1" date={date1} onPress={() => setShowDate1Picker(true)} />
                    <DatePickerBtn label="Fecha 2" date={date2} onPress={() => setShowDate2Picker(true)} />
                </View>

                <View style={styles.rowContainer}>
                    <TimePickerBtn label="Desde" time={startTime} onPress={() => setShowStartTimePicker(true)} />
                    <TimePickerBtn label="Hasta" time={endTime} onPress={() => setShowEndTimePicker(true)} />
                </View>
                ...
                {showStartTimePicker && <DateTimePicker value={startTime} mode="time" is24Hour={true} onChange={(e, d) => { setShowStartTimePicker(Platform.OS === 'ios'); if (d) setStartTime(d); }} />}
                {showEndTimePicker && <DateTimePicker value={endTime} mode="time" is24Hour={true} onChange={(e, d) => { setShowEndTimePicker(Platform.OS === 'ios'); if (d) setEndTime(d); }} />}
                ...
                <TouchableOpacity
                    ...
                    disabled={loading}
                >
                    <Text style={styles.compareBtnText}>{loading ? "Cargando..." : "Comparar"}</Text>
                </TouchableOpacity>
            </View>
